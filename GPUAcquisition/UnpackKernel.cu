...
__global__ void unpack_data(const unsigned char* const raw_data, cufftReal* const chan_a, cufftReal* const chan_b, const size_t num_bytes) {
	...
	chan_a[(idx / 3) * 2] = ((static_cast<float>(a_data) / 4096.f) - 0.5f) * 2.f * 0.400f; // 400 mV
	chan_a[(idx / 3) * 2 + 1] = ((static_cast<float>(b_data)/4096.f) - 0.5f) * 2.f * 0.400f; // 400 mV
...
void gpu_unpack_data(const unsigned char* const raw_data, cufftReal* const chan_a, cufftReal* const chan_b, const size_t num_bytes, const cudaStream_t stream) {
	...
	unpack_data<<<num_blocks, threads_per_block, 0, stream>>>(raw_data, chan_a, chan_b, num_bytes);
}